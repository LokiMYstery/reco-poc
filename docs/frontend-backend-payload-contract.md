...
| `place_candidates` | array | P1 | 推荐 | 最多 3 项，按 confidence 降序；见下方示例 | 可不传，兼容旧版单值字段 | 推荐新版字段；保留附近混合 POI 信息 |
| `place_type` | string | P1 | 兼容保留 | `任意`、`住宅区`、`商场`、`酒店`、`餐厅`、`公园`、`写字楼`、`机场`、`图书馆`、`海边`、`户外`、`在途`、`高铁站`、`地铁站`、`运动场所` | 没权限传 `任意` | 旧版单值字段；传 candidates 时后端自动取 Top-1 |
...
| 字段 | 类型 | 是否建议传 | 说明 |
|---|---|---:|---|
| `place_type` | string | 必传 | 15 个地点枚举之一 |
| `confidence` | float | 必传 | `0.0`-`1.0` |
| `distance_m` | float | 建议 | 用户当前位置到候选 POI 的距离 |
| `source` | string | 建议 | 例如 `mapkit_category`、`poi_name_keyword`、`mapkit_query_probe` |
| `quality` | string | 可选 | `exact_or_good_mapping`、`noisy_mapping` |

后端处理：

- 实时规则按排名权重 `1.0 / 0.45 / 0.20` 软融合 Top-3。
- 后端自动将 Top-1 写入兼容字段 `place_type`、`place_type_confidence`。
- Top-1 与 Top-2 的 confidence 差值 `< 0.12` 时，后端将地点标记为 `noisy_mapping`，不写入细粒度地点历史桶。
- 旧版前端仍可只传单值 `place_type`，接口保持兼容。
...
| 第 3 步 | 地点上下文 | `place_candidates`、`place_type`、`place_type_available`、`place_type_confidence`、`place_type_quality` |
...
| `place_candidates` | array | 推荐 | Top-3 结构见上文 | CoreLocation/POI 映射 | 推荐新版字段，保留混合 POI 信息 |
| `place_type` | string | 兼容保留 | `住宅区` / `在途` | CoreLocation/POI 映射 | 旧版单值字段；后端可由 Top-1 自动补齐 |
