...
struct SetupView: View {
        ...
        VStack(alignment: .leading, spacing: 8) {
          HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: 4) {
              Text(model.capabilityGate.title)
                .font(.headline)
              Text(model.capabilityGate.summary)
                .foregroundStyle(.secondary)
            }
            Spacer()
            StatusBadge(
              text: model.capabilityGate.readiness.rawValue,
              tint: tint(for: model.capabilityGate.readiness))
          }
          Text(model.capabilityGate.detail)
            .font(.subheadline)
            .foregroundStyle(.secondary)
        }
      } header: {
  ...
  private func tint(for readiness: CapabilityReadiness) -> Color {
    switch readiness {
    case .available: .green
    case .limited: .orange
    case .blocked, .requiresHost: .red
    case .optional: .blue
    }
  }
}
